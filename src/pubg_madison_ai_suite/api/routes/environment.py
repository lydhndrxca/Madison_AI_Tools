    ...
    try:
        ...
        if user_context:
            context_block = (
                ...
                "lighting qualities, or environmental details — render those with maximum realism and specificity. "
        ...
        restore_prompt = (
            ...
            "blended surfaces that lack material specificity, vague atmospheric effects. "
            "Your job: recreate this SAME EXACT scene in the SAME EXACT composition — but rendered as if it were "
            "a real photograph or AAA production matte painting. Every surface must be a SPECIFIC, REAL material.\n\n"
            f"{context_block}"
            ...
            "- Architectural surfaces must be SPECIFIC materials — quarried granite, fired brick, rough-hewn timber, poured concrete\n"
            "- Ground surfaces must be identifiable — packed earth, cobblestone, gravel, flagstone — not vague brown\n"
            "- Vegetation must have individual leaf shapes, bark texture, grass blade definition\n"
            "- Metal must show proper reflections, oxidation, and surface finish\n"
            "- Water must show proper refraction, surface tension, and caustics\n"
            "- Atmospheric effects (fog, haze, god-rays) must be physically plausible, not blurry wash\n"
            "- Hard edges where structural elements meet — walls to floors, beams to columns, rooflines\n"
            "- Environmental storytelling details rendered sharply — cracks, stains, moss, rust, weathering\n"
            "- Sky must be clean with resolved cloud forms, not vague gradients\n\n"
...
async def extract_attributes(body: EnvAttributeRequest):
    if not body.description.strip() and not body.image_b64:
        return EnvAttributeResponse(error="Provide a description or image")
    loop = asyncio.get_event_loop()
