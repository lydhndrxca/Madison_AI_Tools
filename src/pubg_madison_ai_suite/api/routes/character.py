    ...
    try:
        ...
        if user_context:
            context_block = (
                ...
                "Incorporate the above direction into the restoration. If the user describes specific materials, "
                "surface qualities, or details — render those with maximum realism and specificity. "
                "Use this as guidance for what to emphasize, refine, and make tangible in the restored image.\n\n"
            )

        restore_prompt = (
            "QUALITY RESTORATION — RECREATE THIS EXACT IMAGE AT MAXIMUM FIDELITY.\n\n"
            "The reference image is a raw AI render with typical generation artifacts: mushy/soupy undefined areas, "
            "blended surfaces that lack material specificity, vague details that should be physically concrete. "
            "Your job: recreate this SAME EXACT subject in the SAME EXACT composition — but rendered as if it were "
            "a real photograph or AAA cinematic still. Every surface must look like it is made of a SPECIFIC, REAL material.\n\n"
            f"{context_block}"
            ...
            "WHAT MUST BE FRESHLY RENDERED (eliminate ALL soupiness):\n"
            "- Every surface must have a clearly identifiable REAL material — not vague blended mush\n"
            "- Metal must look like SPECIFIC metal (brushed steel, tarnished brass, polished chrome) with correct reflections\n"
            "- Leather must show grain, pores, creases, stitching holes\n"
            "- Fabric must show weave pattern, thread texture, proper drape and folds with crisp creases\n"
            "- Skin must show pores, subsurface scattering, micro-detail — not smooth plastic\n"
            "- Hair must be individual strands with proper light interaction, not a blended mass\n"
            "- Hard edges where surfaces meet — no soft blending between materials that should have distinct boundaries\n"
            "- Buckles, rivets, clasps, buttons must be precisely modeled with sharp geometry\n"
            "- Straps must show tension, thickness, edge definition\n"
            "- Background must be clean, resolved, free of noise or compression\n\n"
